...
import 'package:todo_task/fetchers/get_task_fetcher.dart';
import 'package:todo_task/fetchers/url_endpoint.dart';
import 'package:todo_task/model/todo.dart';

class GetTaskModel extends ChangeNotifier{
 ...
 void updateTask(Todo todo){
  ...
  void getTask(bool isLocal)async{
    ...
    QueryResult result = 
    await _client.value.mutate(MutationOptions(document: gql(GetAllTask.getTask,), variables: {
    
     'developer_id':developer_id,
     },fetchPolicy:isLocal ? null: FetchPolicy.networkOnly,
    ));
    if(result.hasException){
      ...
      _status = false;
      _response = "tasks has been gotten";
      notifyListeners();
    }
  }
  void clear(){
